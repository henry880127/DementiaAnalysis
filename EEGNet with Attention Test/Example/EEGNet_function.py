...
from tensorflow.keras.layers import Input, Flatten, Attention, MultiHeadAttention
...
def balanced_accuracy(y_true, y_pred):
    ...
    return (specificity_val+recall_keras)/2.0
...
class EEGNet:
    def __init__(self,
        ...
        self.avgPoolSize_b1 = (1, 4)
        self.avgPoolSize_b2 = (1, 8)
    ...
    def build(self):
        ...
        block1 = Conv2D(self.F1, (1, self.kernLength), padding='same',
                        input_shape=self.input_shape, name='b1_Conv2D',
                        use_bias=False)(input1)
        block1 = BatchNormalization(name='b1_BM1')(block1)
        block1 = DepthwiseConv2D((self.Chans, 1), use_bias=False,
                                 depth_multiplier=self.D, name='b1_DeConv2D',
                                 depthwise_constraint=max_norm(1.))(block1)
        block1 = BatchNormalization(name='b1_BM2')(block1)
        block1 = Activation('elu', name='b1_Act')(block1)
        block1 = AveragePooling2D(
            self.avgPoolSize_b1, name='b1_AvgPool')(block1)
        block1 = Dropout(self.dropout_rate, name='b1_Dropout')(block1)

        block2 = SeparableConv2D(self.F2, (1, self.kernLength//4), name='b2_SeConv2D',
                                 use_bias=False, padding='same')(block1)
        block2 = BatchNormalization(name='b2_BM')(block2)
        block2 = Activation('elu', name='b2_Act')(block2)
        block2 = AveragePooling2D(
            self.avgPoolSize_b2, name='b2_AvgPool')(block2)
        block2 = Dropout(self.dropout_rate, name='b2_Dropout')(block2)

        # Add Attention Layer
        # create query, key, value matrices
        query = Dense(8, name='query')(block2)
        key = Dense(8, name='key')(block2)
        value = Dense(8, name='value')(block2)
        block2 = MultiHeadAttention(num_heads=8, key_dim=8, value_dim=8, name='Attention')(query, key, value)
        

        flatten = Flatten(name='flatten')(block2)

        dense = Dense(self.num_class, name='dense_final',
    ...
    def fit(self, X_train, y_train, X_val, y_val):
        ...
        #                     callbacks=[tensorboard_callback])  # debug:刪除callbacks中的'es'
        history = model.fit(X_train, y_train,
                            batch_size=self.batch_size,
                            shuffle=self.shuffle, epochs=self.epochs,
                            validation_data=(X_val, y_val),
