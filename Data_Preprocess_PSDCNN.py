...
class Data_Preprocess_PSDCNN:
    ...
    def PSD_image(self, data_ndarrays):
        ...
        for key, value in data_ndarrays.items():
            ...
            data_ndarrays_PSD[key] = (self.sec_Window/nDatapoint)*(np.abs(np.fft.fft(value, axis=3))**2)
            ...
            for i in range(nSubjectxEpoch):
                data_ndarrays_PSD[key][i,0,:,:] = ((data_ndarrays_PSD[key][i,0,:,:] - np.min(data_ndarrays_PSD[key][i,0,:,:])) / 
                                                    (np.max(data_ndarrays_PSD[key][i,0,:,:]) - np.min(data_ndarrays_PSD[key][i,0,:,:])))
            print(f'{key}: (subject*epoch,1,channel,datapoint):',data_ndarrays_PSD[key].shape)
